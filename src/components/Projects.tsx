...
const Projects = () => {
  ...
  return (
    <section id="projects" ref={ref} className="section-padding relative overflow-hidden">
      ...
      <div 
        className="absolute top-1/4 -right-32 w-96 h-96 bg-primary/20 rounded-full blur-[120px] animate-pulse-glow"
        style={{ transform: `translateY(${scrollProgress * 60}px)` }}
      />
      <div 
        className="absolute bottom-1/4 -left-32 w-96 h-96 bg-accent/20 rounded-full blur-[120px] animate-pulse-glow"
        style={{ animationDelay: '1.5s', transform: `translateY(${-scrollProgress * 60}px)` }}
      />
      ...
      <div className="section-container relative z-10">
        ...
        <div className="mb-16">
          <div 
            className={`flex items-center gap-3 mb-8 scroll-hidden ${isVisible ? 'scroll-visible' : ''}`}
            style={getStaggeredDelay(1, 100)}
          >
            <div className="p-2 bg-gradient-to-br from-primary to-accent rounded-lg shadow-lg animate-neuron-pulse">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            {featuredProjects.map((project, index) => (
              <Card
                key={index}
                style={getStaggeredDelay(index + 2, 200)}
                className={`glass-card border-2 border-primary/40 hover:border-primary/80 transition-all duration-500 relative overflow-hidden group scroll-hidden ${isVisible ? 'scroll-visible' : ''}`}
              >
                {/* Gradient overlay on hover */}
                <div className="absolute inset-0 bg-gradient-to-br from-primary/15 to-accent/15 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                
                {/* Strong neon border glow on hover */}
                <div 
                  className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                  style={{ boxShadow: 'inset 0 0 40px hsl(190 100% 50% / 0.15), 0 0 40px hsl(190 100% 50% / 0.3), 0 0 60px hsl(190 100% 50% / 0.15)' }}
                />
                
                <CardHeader className="relative">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <Badge className="mb-3 bg-gradient-to-r from-primary to-accent text-primary-foreground border-0 shadow-lg shadow-primary/30">
                ...
                <CardContent className="space-y-4 relative">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.highlights.map((highlight) => (
                      <Badge key={highlight} variant="outline" className="text-xs font-semibold border-primary/60 bg-primary/15 text-primary">
                        {highlight}
                      </Badge>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3 pt-2">
                    <Button asChild size="sm" className="gap-1.5 bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 shadow-lg shadow-primary/30 hover:shadow-primary/50 transition-all duration-300 hover:scale-105">
                    ...
                    {project.liveDemo && (
                      <Button asChild variant="outline" size="sm" className="gap-1.5 border-primary/50 hover:bg-primary/15 hover:border-primary transition-all duration-300 hover:scale-105">
                    ...
                    <Button asChild variant="outline" size="sm" className="gap-1.5 border-border/50 hover:bg-secondary hover:scale-105 transition-all duration-300">
        ...
        <div>
          <h3 
            className={`text-xl font-semibold mb-8 scroll-hidden ${isVisible ? 'scroll-visible' : ''}`}
            style={getStaggeredDelay(4, 100)}
          >
            More Projects
          </h3>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {otherProjects.map((project, index) => (
              <Card
                key={index}
                style={getStaggeredDelay(index + 5, 150)}
                className={`glass-card border border-border/40 hover:border-primary/50 transition-all duration-500 group scroll-hidden-scale ${isVisible ? 'scroll-visible-scale' : ''}`}
              >
                {/* Glow on hover */}
                <div 
                  className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                  style={{ boxShadow: '0 0 30px hsl(190 100% 50% / 0.2)' }}
                />
                ...
                <CardContent className="space-y-4 relative">
                  ...
                  <Button asChild variant="outline" size="sm" className="gap-1.5 w-full border-border/50 hover:border-primary/60 hover:bg-primary/10 transition-all duration-300">
                    <a href={project.github} target="_blank" rel="noopener noreferrer">
                      <Github className="h-3.5 w-3.5" />
                      View on GitHub
                    </a>
